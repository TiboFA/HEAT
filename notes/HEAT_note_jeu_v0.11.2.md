...
## v0.11.2 — la palette sombre passe le test de lisibilité

Consigne reçue : *« charte sombre, pas assez lisible. »* Vérification faite plutôt que devinée : sur les onze couleurs de texte de la palette, quatre ne passaient pas le seuil de contraste AA (4,5:1) sur au moins un des quatre fonds sombres du jeu.

### Ce qui ne passait pas

Mesuré à la formule WCAG, sur les quatre fonds réellement utilisés (`--plane`, `--surface`, `--surface-2`, `--sea`) :

| Couleur | Usage | Pire contraste mesuré | Seuil |
|---|---|---|---|
| `--s-fric` (rouge friction) | texte — malus, alertes, pièges | 3,31:1 sur `--surface-2` | échoue partout |
| `--s-autres` (orange) | texte — dégâts secondaires | 4,10:1 sur `--surface-2` | échoue sur deux fonds |
| `--s-traj` (bleu trajectoire) | texte — combos, chiffres clefs | 4,38:1 sur `--surface-2` | échoue sur un fond |
| `--muted` (gris secondaire) | texte — sous-titres, légendes, dates | 4,43:1 sur `--surface-2` | échoue sur un fond |

`--s-fric` est le pire cas et le plus visible : c'est la couleur des lignes de pénalité, des pièges et de la friction — exactement le texte qu'un joueur a besoin de lire sans effort pour comprendre pourquoi un coup lui coûte cher.

### La correction, et pourquoi elle ne touche pas tout

Ces mêmes couleurs servent aussi de fond plein derrière du texte blanc (badges, bouton « fin de tour » armé, remplissages de jauge). Les éclaircir partout aurait réparé la lisibilité du texte en cassant celle des badges — mesuré : le rouge d'origine passe *déjà* difficilement en fond (4,8:1 avec du blanc dessus), l'éclaircir le fait tomber à 3,5:1.

Solution retenue : trois variantes de texte, `--s-fric-tx`, `--s-traj-tx`, `--s-autres-tx`, réservées aux déclarations `color:` — jamais aux fonds, bordures ou ombres, qui gardent la couleur d'origine, plus saturée. `--muted` est en revanche relevé directement : il ne sert jamais de fond dans la feuille de style, aucun compromis à faire.

| Variable | Avant | Après | Contraste sur `--surface-2` (pire cas) |
|---|---|---|---|
| `--muted` | `#898781` | `#a19f98` | 4,43 → 6,01 |
| `--s-fric-tx` | `#d03b3b` (partagé) | `#e57979` | 3,31 → 5,57 |
| `--s-traj-tx` | `#3987e5` (partagé) | `#5c9eeb` | 4,38 → 5,72 |
| `--s-autres-tx` | `#d95926` (partagé) | `#e8875e` | 4,10 → 6,11 |

Les quatre couleurs passent maintenant 4,5:1 sur les quatre fonds du jeu, avec de la marge. Les badges, boutons et remplissages n'ont pas changé de teinte.

### Ce que ça change à l'écran

Rien ne bouge de place — c'est une passe de couleur, pas de mise en page. Le texte rouge des pièges et des malus, l'orange des dégâts secondaires, le bleu des combos et le gris des légendes sont plus clairs. Vérifié sur l'accueil, le plateau en cours de partie et une fiche de levier ouverte.

**La version de sauvegarde ne bouge pas.** `SAVE_V` reste à `0.11` : ni l'écran d'accueil (déjà séparé en v0.11.1) ni la palette ne touchent au format d'une partie enregistrée.
